# Developer Guide: Sovereign Supervisor Dispatch
...
```bash
# 1. Clone
git clone https://github.com/BlackFoxgamingstudio/supervisor-dispatch.git
...
```bash
export SBB_MAPS_API_KEY=your_value_here
export SBB_SLA_TARGETS_PATH=your_value_here
export SBB_DISPATCH_ALGO=your_value_here
export SBB_GEO_RADIUS_KM=your_value_here
...
│   ├── cli.py               # Typer CLI (sovereign-supervisor-dispatch)
...
```bash
# Build
docker build -t sovereign-supervisor-dispatch:dev .

# Run with env file
docker run --env-file .env -p 8792:8792 sovereign-supervisor-dispatch:dev

# Full stack (with n8n network)
docker-compose up --build

# Logs
docker-compose logs -f sovereign-supervisor-dispatch
...
3. Use **SovereignTools** node → configure endpoint to `http://localhost:8792`
...
requests.post(N8N_EVENT_BUS_URL, json={
    "source": "sovereign-supervisor-dispatch",
...
| Issue | Diagnosis | Fix |
|-------|-----------|-----|
| Port `8792` in use | `lsof -i :8792` | Kill PID or change `SERVICE_PORT` |
